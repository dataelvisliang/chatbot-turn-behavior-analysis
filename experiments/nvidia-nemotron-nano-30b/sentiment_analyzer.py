"""
Task 3: Sentiment Gradient Analysis (Mental Health Q&A)
...
Note: Longer conversations (5-25 turns) provide richer gradient data
...
class SentimentGradientAnalyzer:
    ...
    def _extract_session_features(
        ...
        return {
            ...
            'highest_point': float(np.max(smoothed)),
            'highest_point_turn': int(np.argmax(smoothed)) + 1,
    ...
    def print_summary(self, results: List[Dict]):
        ...
        for r in results:
            ...
            all_turn_counts.append(r.get('num_turns', 0))
        ...
        print(f"Average turns per conversation: {np.mean(all_turn_counts):.1f}")
        print(f"Turn range: {np.min(all_turn_counts)} - {np.max(all_turn_counts)}")
        
        print("\n" + "-" * 40)
        print("OVERALL TRENDS:")
        print("-" * 40)
        print(f"Improving (slope > 0):  {improving:5d} ({100*improving/max(1,total):5.1f}%)")
        print(f"Stable:                 {stable:5d} ({100*stable/max(1,total):5.1f}%)")
        print(f"Worsening (slope < 0):  {worsening:5d} ({100*worsening/max(1,total):5.1f}%)")
        print(f"With recovery:          {recoveries:5d} ({100*recoveries/max(1,total):5.1f}%)")
